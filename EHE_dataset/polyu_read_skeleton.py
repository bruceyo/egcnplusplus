...
def read_xyzh(file, max_body=2, num_joint=25):
    seq_info = read_skeleton(file)
    data = np.zeros((4, seq_info['numFrame'], num_joint, max_body))
    for n, f in enumerate(seq_info['frameInfo']):
        for m, b in enumerate(f['bodyInfo']):
            for j, v in enumerate(b['jointInfo']):
                if m < max_body and j < num_joint:
                    data[:, n, j, m] = [v['x'], v['y'], v['z'], v['h']]
                else:
                    pass
    return data
...
def read_ang(file, max_body=2, num_joint=25):
    ...
    for n, f in enumerate(seq_info['frameInfo']):
        for m, b in enumerate(f['bodyInfo']):
            for j, v in enumerate(b['jointInfo']):
                if m < max_body and j < num_joint:
                    data[:, n, j, m] = [v['ori_x'], v['ori_y'], v['ori_z']]
                else:
                    pass
    return data

def read_xyz_angw(file, max_body=2, num_joint=25):
    seq_info = read_skeleton(file)
    data = np.zeros((4, seq_info['numFrame'], num_joint, max_body))
    for n, f in enumerate(seq_info['frameInfo']):
        for m, b in enumerate(f['bodyInfo']):
            for j, v in enumerate(b['jointInfo']):
                if m < max_body and j < num_joint:
                    data[:, n, j, m] = [v['ori_x'], v['ori_y'], v['ori_z'], v['ori_w']]
                else:
                    pass
    return data
...
def read_xyzang(file, max_body=2, num_joint=25):
    ...
    for n, f in enumerate(seq_info['frameInfo']):
        for m, b in enumerate(f['bodyInfo']):
            for j, v in enumerate(b['jointInfo']):
                if m < max_body and j < num_joint:
                    data[:, n, j, m] = [v['x'], v['y'], v['z'], v['ori_x'], v['ori_y'], v['ori_z']]
                else:
                    pass
    return data

def read_xyzhangw(file, max_body=2, num_joint=25):
    seq_info = read_skeleton(file)
    data = np.zeros((8, seq_info['numFrame'], num_joint, max_body))
    for n, f in enumerate(seq_info['frameInfo']):
        for m, b in enumerate(f['bodyInfo']):
            for j, v in enumerate(b['jointInfo']):
                if m < max_body and j < num_joint:
                    data[:, n, j, m] = [v['x'], v['y'], v['z'], v['h'],v['ori_x'], v['ori_y'], v['ori_z'], v['ori_w']]
                else:
                    pass
    return data
